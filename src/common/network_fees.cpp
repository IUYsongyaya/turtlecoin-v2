...
namespace TurtleCoin::Common::NetworkFees
{
    ...
    size_t calculate_transaction_discount(size_t leading_zeros)
    {
         ...
         */
        return std::min(
            leading_zeros - TurtleCoin::Configuration::Transaction::Fees::MINIMUM_POW_ZEROS,
            TurtleCoin::Configuration::Transaction::Fees::MAXIMUM_FEE_DISCOUNT);
    }
    ...
    uint64_t calculate_transaction_fee(size_t transaction_size, size_t leading_zeros)
    {
        ...
        const auto discount = calculate_transaction_discount(leading_zeros);

        // calculate the discount as a percentage
        const auto discount_percentage = float(discount) / 100;
